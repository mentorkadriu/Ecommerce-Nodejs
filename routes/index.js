...
router.get("/products", function (req, res, next) {
  ...
  Product.getAllProducts(
    {
      ...query,
      title: { $regex: new RegExp("^" + queryTitle.toLowerCase(), "i") },
    },
    order,
    function (e, products) {
      if (e) {
...
router.get("/filters", async function (req, res, next) {
  const filters = {};
  await Product.find().distinct("color", function (err, color) {
    if (err) return next(err);
    filters.colors = color;
    Product.find().distinct("size", function (err, size) {
      if (err) return next(err);
      filters.sizes = size;
      Product.find().distinct("brand", function (err, brand) {
        if (err) return next(err);
...
router.get("/search", function (req, res, next) {
  ...
  Product.getProductByTitle(
    {
      ...query,
      title: { $regex: new RegExp("^" + query.title.toLowerCase(), "i") },
    },
    order,
    function (err, p) {
